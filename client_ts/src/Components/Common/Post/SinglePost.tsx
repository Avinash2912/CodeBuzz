...
type PropsType = {data?:PostType,children?:React.ReactNode,isSinglePost?:boolean};
export default function SinglePost(props:PropsType)
{
    const auth = useAuth();
    ...
    return(
        <>
            <div className='card'>
                <h1 className='font-bold text-xl mt-4'>{isPlaying}</h1>
                ...
                <div className={`${isPlaying?"h-16":"h-0"} transition-all duration-500`}/>
            </div>
            <div className={`${isPlaying?(isVisible?"p-4 border-white/0 -mt-20 m-4 h-16":"shadow-lg card -mt-20 h-16"):"h-0"} bg-dark transition-all duration-500 sticky bottom-0 top-0 z-10 overflow-hidden flex justify-between`}>
                <button onClick={playSound} className='flex items-center transition-all active:scale-90'>
